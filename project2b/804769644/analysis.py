...
shp_info = m.readshapefile('/Users/mnallamalli97/Desktop/143/shared_folder/p2bplots/st99_d00','states',drawbounds=True)  # No extension specified in path here.
...
for shapedict in m.states_info:
    ...
    if statename not in ['District of Columbia', 'Puerto Rico']:
        ...
        pos_colors[statename] = pos_cmap(100*(1. - np.sqrt(( pos - vmin )/( vmax - vmin))))[:3]
...
for shapedict in m.states_info:
    ...
    if statename not in ['District of Columbia', 'Puerto Rico']:
        ...
        neg_colors[statename] = neg_cmap(100*(1. - np.sqrt(( neg - vmin )/( vmax - vmin))))[:3]
    statenames.append(statename)
# cycle through state names, color each one.
...
"""
PART 3 difference of pos - neg
"""
vmin = -100; vmax = 0 # set range.
for shapedict in m.states_info:
    statename = shapedict['NAME']
    # skip DC and Puerto Rico.
    if statename not in ['District of Columbia', 'Puerto Rico']:
        diff = diff_data[statename]
        diff_colors[statename] = diff_cmap(100*(1. - np.sqrt(( diff - vmin )/( vmax - vmin))))[:3]
    statenames.append(statename)
# cycle through state names, color each one.
